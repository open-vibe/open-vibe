...
pub(crate) fn build_menu<R: tauri::Runtime>(
    ...
    let new_agent_item = MenuItemBuilder::with_id("file_new_agent", "新建代理").build(handle)?;
    ...
    let cycle_access_item =
        MenuItemBuilder::with_id("composer_cycle_access", "切换权限模式")
    ...
    let toggle_git_sidebar_item =
        MenuItemBuilder::with_id("view_toggle_git_sidebar", "切换 Git 侧栏")
            .build(handle)?;
